...
import { limitRequest, tooManyRequests } from "../../../lib/rateLimit";
...
const validDate = (value: unknown) => { const text=String(value||""); if(!/^\d{4}-\d{2}-\d{2}$/.test(text))return false; const parsed=new Date(`${text}T00:00:00.000Z`); return !Number.isNaN(parsed.getTime())&&parsed.toISOString().slice(0,10)===text; };
...
export async function GET(_request:Request,{params}:{params:Promise<{token:string}>}){
 try{const {token}=await params,limit=await limitRequest(_request,"portal-read",token,60,60);if(!limit.allowed)return tooManyRequests(limit.retryAfter);const sql=portalDb(),access=await accessFor(token);if(!access)return Response.json({error:"This portal link is invalid, expired, or disabled."},{status:404});
...
export async function POST(request:Request,{params}:{params:Promise<{token:string}>}){
 try{const {token}=await params,limit=await limitRequest(request,"portal-submit",token,10,15*60);if(!limit.allowed)return tooManyRequests(limit.retryAfter);const sql=portalDb(),access=await accessFor(token);if(!access)return Response.json({error:"This portal link is invalid, expired, or disabled."},{status:404});const body=await request.json(),rows=Array.isArray(body.readings)?body.readings:[];
  if(!rows.length)return Response.json({error:"Enter at least one glucose value."},{status:400});
  if(rows.length>31)return Response.json({error:"A submission may contain no more than 31 dated rows."},{status:413});
  const dates=new Set<string>();
  for(const row of rows){if(!validDate(row.date))return Response.json({error:"Every reading needs a valid date."},{status:400});if(dates.has(String(row.date)))return Response.json({error:"Each date may appear only once per submission."},{status:400});dates.add(String(row.date));for(const key of ["fasting","breakfast","lunch","dinner"]){const value=numeric(row[key]);if(value!=null&&(!Number.isFinite(value)||value<20||value>999))return Response.json({error:"Glucose values must be between 20 and 999 mg/dL."},{status:400});}}

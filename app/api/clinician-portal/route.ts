...
async function snapshot(){
 ...
 return {
  ...
  readings:importedRows.map(r=>({id:String(r.id),patientId:String(r.patient_id),date:String(r.reading_date).slice(0,10),fasting:n(r.fasting),breakfast:n(r.breakfast),lunch:n(r.lunch),dinner:n(r.dinner),notes:String(r.notes||""),source:"Patient Portal",sourceSubmissionId:r.source_submission_id?String(r.source_submission_id):undefined}))
 };
}
...
export async function PATCH(request:Request){
 try{const body=await request.json(),sql=portalDb(),provider=String(body.provider||"Prototype Clinician"),submissionId=String(body.submissionId||""),submission=(await sql`SELECT * FROM patient_submissions WHERE id=${submissionId} LIMIT 1`)[0];if(!submission)return Response.json({error:"Submission not found"},{status:404});
  ...
  if(body.action==="import"){
   ...
   for(const row of rows){const existing=(await sql`SELECT * FROM patient_glucose_readings WHERE patient_id=${submission.patient_id} AND reading_date=${row.reading_date} LIMIT 1`)[0],choice=(body.choices||{})[String(row.reading_date).slice(0,10)] as Choice|undefined,hasDuplicate=existing&&["fasting","breakfast","lunch","dinner"].some(k=>existing[k]!=null&&row[k]!=null);if(hasDuplicate&&!choice)return Response.json({error:"Choose how to handle each possible duplicate before importing.",duplicateDate:String(row.reading_date).slice(0,10)},{status:409});if(choice==="skip")continue;
    ...
    }else await sql`INSERT INTO patient_glucose_readings (id,patient_id,reading_date,fasting,breakfast,lunch,dinner,notes,source,source_submission_id) VALUES (${id()},${submission.patient_id},${row.reading_date},${n(row.fasting)},${n(row.breakfast)},${n(row.lunch)},${n(row.dinner)},${String(row.notes||"")},'Patient Portal',${submissionId})`;}
   await sql`UPDATE patient_submissions SET status='Imported',approved_by=${provider},approved_at=now() WHERE id=${submissionId} AND status='Pending'`;await sql`INSERT INTO audit_events (id,patient_id,submission_id,action,actor,details) VALUES (${id()},${submission.patient_id},${submissionId},'Clinician approved/imported submission',${provider},'Imported to permanent glucose log')`;return Response.json(await snapshot());
  }
